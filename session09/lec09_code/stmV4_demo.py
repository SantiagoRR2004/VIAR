...
class STMEncoder(nn.Module):
    ...
    def __init__(self, in_channels=3, key_channels=128, value_channels=256):
        ...
        self.value_encoder = nn.Sequential(
            nn.Conv2d(in_channels + 1, 64, 7, stride=2, padding=3),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(3, stride=2, padding=1),
            nn.Conv2d(64, 128, 3, padding=1),
            nn.ReLU(inplace=True),
...
class MemoryBank:
    ...
    def get_all_keys(self):
        return None if not self.keys else torch.cat(self.keys, dim=2)

    def get_all_values(self):
        return None if not self.values else torch.cat(self.values, dim=2)

    def clear(self):
        self.keys, self.values, self.frame_ids = [], [], []
...
class MemoryReader(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.Ck, self.Cv, self.top_k, self.tau = (
            key_channels,
            value_channels,
            top_k,
            max(1e-4, float(tau)),
...
class STM(nn.Module):
    ...
    def __init__(
        self, key_channels=128, value_feat_channels=256, memory_size=20, top_k=128
    ):
        super().__init__()
        # +1 channel at the end for raw mask
        self.encoder = STMEncoder(3, key_channels, value_feat_channels)
        self.reader = MemoryReader(
            key_channels, value_feat_channels + 1, top_k=32, tau=0.07
        )
    ...
    @torch.no_grad()
    def _downsample_mask_to(
        self, mask: torch.Tensor, like: torch.Tensor
    ) -> torch.Tensor:
    ...
    def segment_frame(self, image: torch.Tensor, frame_id: int) -> torch.Tensor:
        ...
        mask_hr = F.interpolate(
            read_mask_lr, size=image.shape[2:], mode="bilinear", align_corners=False
        )
        return mask_hr
...
def _read_video_frames(path, max_frames=999999, resize_short=360):
    ...
    while len(frames) < max_frames:
        ...
        if resize_short is not None:
            if H < W:
                newH, newW = resize_short, int(W * (resize_short / H))
            else:
                newW, newH = resize_short, int(H * (resize_short / W))
...
def _overlay_video(frames_bgr, masks, out_path, fps=30, alpha=0.45):
    ...
    for f_bgr, m in zip(frames_bgr, masks):
        ...
        vis = f_bgr.copy()
        red = np.zeros_like(f_bgr)
        red[..., 2] = 255
        vis[m_bin > 0] = (alpha * red[m_bin > 0] + (1 - alpha) * vis[m_bin > 0]).astype(
            np.uint8
        )
        vw.write(vis)
...
def _iou_tensor(a: torch.Tensor, b: torch.Tensor) -> float:
    ...
    b = (b >= 0.5).float()
    inter = (a * b).sum()
    union = (a + b - a * b).clamp_min(1e-6).sum()
...
def process_video_adaptive(
    ...
    flow_thr=2.5,
    iou_thr=0.75,
    conf_thr=0.40,  # conf_thr kept for symmetry (we use mean prob of read mask)
    annotate: bool = False,
):
    ...
    H, W = frames_bgr[0].shape[:2]
    fps = _probe_fps(video_path, default=30)

    # Build initial mask
    if annotate:
        print(
            "[STM] Annotation UI: draw one or more rectangles; ENTER to finish, 'r' to reset."
        )
        m0 = annotate_first_frame(frames_bgr[init_frame])
    elif init_mask_path:
        m0 = cv2.imread(init_mask_path, cv2.IMREAD_GRAYSCALE)
        if m0 is None:
            raise RuntimeError(f"Could not read init mask: {init_mask_path}")
        m0 = (cv2.resize(m0, (W, H), interpolation=cv2.INTER_NEAREST) > 127).astype(
            np.float32
        )
    elif anno_boxes:
        m0 = _mask_from_boxes_json(anno_boxes, init_frame, H, W)
        if m0.sum() == 0:
            raise RuntimeError(f"No boxes for frame {init_frame} in {anno_boxes}")
    else:
    ...
    stm = STM(
        key_channels=64, value_feat_channels=256, memory_size=memory_size, top_k=top_k
    ).to(device)
    stm.eval()
    ...
    for i in range(init_frame + 1, len(frames_t)):
        ...
        if (i - init_frame) % 20 == 0:
            print(
                f"[STM] frame {i}: mean_conf={mean_conf:.3f}, iou_to_last={iou_to_last:.3f}, flow={flow_mean:.2f}"
            )
...
def main():
    ap = argparse.ArgumentParser(
        "STM VOS — training-free, with raw-mask propagation and annotator"
    )
    ap.add_argument("--video_path", type=str, required=True)
    ap.add_argument(
        "--init_mask", type=str, default=None, help="PNG of first-frame mask"
    )
    ap.add_argument(
        "--anno_boxes", type=str, default=None, help="Boxes JSON for init frame"
    )
    ...
    ap.add_argument(
        "--annotate",
        action="store_true",
        help="Open a simple UI to draw the first mask",
    )
