...
class SimpleBackbone(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer3 = nn.Sequential(
            nn.MaxPool2d(2), nn.Conv2d(512, 1024, 3, padding=1), nn.ReLU()
        )  # C4: stride 16

        self.layer4 = nn.Sequential(
            nn.MaxPool2d(2), nn.Conv2d(1024, 2048, 3, padding=1), nn.ReLU()
        )  # C5: stride 32
...
def visualize_fpn_architecture():
    ...
    for i in range(len(levels) - 1):
        arrow = FancyArrowPatch(
            (1.5, levels[i]["y"] + levels[i]["size"] / 2),
    ...
    for i, level in enumerate(levels):
        ...
        arrow = FancyArrowPatch(
            (2.5, level["y"]), (3, level["y"]), arrowstyle="->", lw=1.5, color="blue"
        )
        ax.add_patch(arrow)
    ...
    for i, level in enumerate(levels):
        ...
        arrow = FancyArrowPatch(
            (4, level["y"]), (4.5, level["y"]), arrowstyle="->", lw=1.5, color="green"
        )
        ax.add_patch(arrow)
    ...
    for i, level in enumerate(levels):
        ...
        # Arrow to final
        arrow = FancyArrowPatch(
            (7.5, level["y"]), (8, level["y"]), arrowstyle="->", lw=1.5, color="black"
        )
        ax.add_patch(arrow)
    ...
    for x, y, name in pyramid_features:
        # Arrow to detection
        arrow = FancyArrowPatch(
            (x + 0.6, y), (10, y), arrowstyle="->", lw=1.5, color="black"
        )
        ax.add_patch(arrow)
    ...
    ax.text(
        0.5, legend_y, "• High resolution (P3) + Strong semantics (from P5)", fontsize=9
    )
    ax.text(
        0.5,
        legend_y - 0.4,
        "• Multi-scale detection (P3→P5 for small→large)",
        fontsize=9,
    )
