...
class RoIPooling(nn.Module):
    ...
    def forward(self, feature_map, rois, image_size):
        ...
        pooled_features = torch.zeros(
            num_rois, channels, output_h, output_w, device=feature_map.device
        )
            ...
            for i in range(output_h):
                for j in range(output_w):
                    ...
                    if y_end > y_start and x_end > x_start:
                        roi_bin = feature_map[
                            batch_idx, :, y_start:y_end, x_start:x_end
                        ]
...
class FastRCNN(nn.Module):
    ...
    def __init__(self, num_classes=20, roi_output_size=(7, 7)):
        ...
        # Detection head
        in_features = (
            self.backbone_out_channels * roi_output_size[0] * roi_output_size[1]
        )
...
class FastRCNNLoss:
    ...
    def __call__(
        self, cls_scores, bbox_deltas, labels, bbox_targets, bbox_inside_weights
    ):
        """
        Args:
            cls_scores: [N, num_classes+1]
...
class FastRCNNTrainer:
    ...
    def prepare_training_data(self, images, annotations, proposals_list):
        ...
        for batch_idx, (proposals, annot) in enumerate(
            zip(proposals_list, annotations)
        ):
            ...
            for proposal in proposals:
                ...
                if max_iou >= 0.5:
                    ...
                    bbox_target = self.compute_bbox_targets(
                        proposal, gt_boxes_xyxy[best_gt_idx]
                    )
...
def demo_fast_rcnn():
    ...
    for epoch in range(10):
        ...
        print(
            f"   Epoch {epoch+1:2d} | Loss: {total_loss:.4f} "
    ...
    img_tensor = (
        torch.from_numpy(test_img).permute(2, 0, 1).float().unsqueeze(0) / 255.0
    )
    img_tensor = img_tensor.to(device)

    rois = torch.tensor([[0] + prop for prop in proposals], dtype=torch.float32).to(
        device
    )
