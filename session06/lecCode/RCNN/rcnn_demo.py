    ...
    def __init__(self, pretrained=True):
        ...
        self.transform = transforms.Compose(
            [
                ...
                transforms.Resize(
                    (224, 224)
                ),  # R-CNN used 227x227, we use 224x224 for ResNet
                transforms.ToTensor(),
    ...
    def train_stage2_svm(self, images, annotations):
        ...
        for class_id in range(self.num_classes):
            ...
            if len(unique) < 2 or pos == 0 or neg == 0:
                print(
                    f"    Skipping class {class_id}: needs >=1 positive and >=1 negative "
            ...
            print(
                f"    Positive samples: {sum(y)}, Negative samples: {len(y) - sum(y)}"
            )
                ...
                for proposal in proposals:
                    ...
                    for gt_box, gt_label in zip(gt_boxes, gt_labels):
                        if gt_label == class_id:
                            iou = self.compute_iou(proposal, gt_box)
                            if iou > 0.6:
                                feature = self.cnn.extract_region_features(
                                    img, proposal
                                )
        ...
        for proposal, feature in zip(proposals, features):
            for class_id in range(self.num_classes):
                ...
                if score > conf_threshold:
                    # Refine box
                    refined_box = self.bbox_regressors.predict(
                        feature, proposal, class_id
                    )
...
def visualize_detections(
    image, detections, class_names=None, save_path="rcnn_detection.png"
):
    ...
    for box, class_id, score in detections:
        ...
        rect = patches.Rectangle(
            (x, y), w, h, linewidth=2, edgecolor=colors[class_id % 20], facecolor="none"
        )
        ax.add_patch(rect)
        ...
        ax.text(
            ...
            fontsize=10,
            bbox=dict(
                boxstyle="round,pad=0.3", facecolor=colors[class_id % 20], alpha=0.7
            ),
        )
...
if __name__ == "__main__":
    ...
    for i, (box, class_id, score) in enumerate(detections):
        print(
            f"  Detection {i+1}: Class {class_id}, Score {score:.3f}, Box {[int(b) for b in box]}"
        )
