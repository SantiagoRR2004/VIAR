...
class MiniSAM(nn.Module):
    ...
    def __init__(self, n_classes=21, embed_dim=256):
        ...
        self.point_pos_embed = nn.Sequential(
            nn.Linear(2, 128), nn.ReLU(), nn.Linear(128, embed_dim)
        )

        # Box encoding
        self.box_embed = nn.Sequential(
            nn.Linear(4, 128), nn.ReLU(), nn.Linear(128, embed_dim)
        )
...
class MiniSAMTrainer:
    ...
    def sample_points_from_mask(masks, n_points=5):
        ...
        for b in range(B):
            ...
            if len(fg_indices) > 0:
                fg_samples = fg_indices[
                    torch.randint(len(fg_indices), (n_points // 2,))
                ]
                fg_points = fg_samples.float() / torch.tensor([H, W]).to(
                    fg_samples.device
                )
            ...
            if len(bg_indices) > 0:
                bg_samples = bg_indices[
                    torch.randint(len(bg_indices), (n_points // 2,))
                ]
                bg_points = bg_samples.float() / torch.tensor([H, W]).to(
                    bg_samples.device
                )
