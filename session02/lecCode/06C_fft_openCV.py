...
def ensure_figs():
    outdir = Path("figs"); outdir.mkdir(parents=True, exist_ok=True); return outdir
...
def apply_mask_and_idft(Fshift: np.ndarray, mask_hw: np.ndarray, out_h: int, out_w: int) -> np.ndarray:
...
def montage_three_rows(a, b, c):
    ...
    def rs(im): return cv2.resize(im, (w, int(im.shape[0]*w/im.shape[1])), interpolation=cv2.INTER_AREA)
    return cv2.vconcat([rs(a), rs(b), rs(c)])

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--input", type=str, required=True, help="Path to input image (grayscale will be used)")
    ap.add_argument("--R", type=int, default=40, help="Ideal low-pass radius (pixels in padded spectrum)")
    ap.add_argument("--notch", type=str, nargs="*", default=["0,70,8","0,-70,8"],
                    help='Notch triplets "dy,dx,rr" in pixels (centered coords)')
    ap.add_argument("--montage", action="store_true", help="Also save a quick montage grid")
    args = ap.parse_args()
    ...
    print(f"[done] Wrote: {figs/'fft_mag.png'}, {figs/'fft_phase.png'}, {figs/'fft_lowpass.png'}, {figs/'fft_notch.png'}"
