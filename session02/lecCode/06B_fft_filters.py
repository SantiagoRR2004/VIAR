...
def panel_spectrum_and_mask(F, Hmask, Ff, title=""):
    ...
    ax1 = fig.add_subplot(1,3,1); ax1.imshow(mag_spectrum(F), cmap='gray'); ax1.set_title("Spectrum |F| (log)"); ax1.axis('off')
    ax2 = fig.add_subplot(1,3,2); ax2.imshow(normalize01(Hmask), cmap='gray'); ax2.set_title("Mask H(u,v)"); ax2.axis('off')
    ax3 = fig.add_subplot(1,3,3); ax3.imshow(mag_spectrum(Ff), cmap='gray'); ax3.set_title("|F·H| (log)"); ax3.axis('off')
    fig.suptitle(title)
    fig.tight_layout()
    return fig
...
def side_by_side(orig01, out01, title_left="Original", title_right="Filtered"):
    ...
    ax1 = fig.add_subplot(1,2,1); ax1.imshow(orig01 if orig01.ndim==3 else orig01, cmap=None if orig01.ndim==3 else 'gray'); ax1.set_title(title_left); ax1.axis('off')
    ax2 = fig.add_subplot(1,2,2); ax2.imshow(out01 if out01.ndim==3 else out01, cmap=None if out01.ndim==3 else 'gray'); ax2.set_title(title_right); ax2.axis('off')
    fig.tight_layout()
    return fig
...
def parse_args():
    ...
    p.add_argument("--filter", choices=["gaussian","ideal","butter","laplacian"], default="gaussian")
    p.add_argument("--type", choices=["lowpass","highpass","bandpass","bandstop"], default="lowpass",
                   help="Ignored for laplacian")
    p.add_argument("--cutoff", type=float, default=0.10, help="Normalized radius (cycles/pixel), e.g., 0.10")
    p.add_argument("--cutoff2", type=float, default=None, help="Second cutoff for band filters")
    p.add_argument("--order", type=int, default=2, help="Butterworth order")
    p.add_argument("--pad", action="store_true", help="Zero-pad to next power of 2 (with reflection boundary)")
    p.add_argument("--compare-spatial", action="store_true", help="Compare Gaussian LP (freq vs spatial)")
    p.add_argument("--sigma-px", type=float, default=2.0, help="Spatial Gaussian sigma (pixels) for comparison")
...
def main():
    ...
    if args.mode == "gray":
        ...
        out01, Hmask, F, Ff = process_gray(gray01,
                                           filt=args.filter, ftype=args.type,
                                           cutoff=args.cutoff, cutoff2=args.cutoff2,
                                           order=args.order, do_pad=args.pad)
        fig1 = side_by_side(gray01, out01, "Original (gray)", f"{args.filter}-{args.type}")
        ...
        if args.save:
            ...
            fig1.savefig(outdir/(path.stem+f"_{args.filter}_{args.type}_pair.png"), dpi=180, bbox_inches="tight")
            fig2.savefig(outdir/(path.stem+f"_{args.filter}_{args.type}_spectra.png"), dpi=180, bbox_inches="tight")
        plt.show(block=False)

        # Optional: compare frequency Gaussian LP with spatial blur
        if args.compare_spatial and args.filter == "gaussian" and args.type == "lowpass":
            sigma_f = approx_sigma_f_from_sigma_px(args.sigma_px)
            print(f"[compare] sigma_px={args.sigma_px:.2f} -> approx cutoff (σ_f)={sigma_f:.4f} cycles/pixel")
            out_freq, _, _, _ = process_gray(gray01, 'gaussian', 'lowpass', sigma_f, None, args.order, args.pad)
            out_spat = gaussian_spatial(gray01, args.sigma_px)
            fig3 = side_by_side(out_spat, out_freq, "Spatial Gaussian", "Freq Gaussian (σ_f≈1/2πσ)")
            if args.save:
                save_gray(outdir/(path.stem+"_gauss_spatial.png"), out_spat)
                save_gray(outdir/(path.stem+"_gauss_freq.png"), out_freq)
                fig3.savefig(outdir/(path.stem+"_gauss_compare.png"), dpi=180, bbox_inches="tight")
            plt.show()

    else:  # color
        rgb01 = normalize01(rgb.astype(np.float64))
        out01, Hmask, F, Ff = process_color(rgb01,
                                            filt=args.filter, ftype=args.type,
                                            cutoff=args.cutoff, cutoff2=args.cutoff2,
                                            order=args.order, do_pad=args.pad)
        fig1 = side_by_side(rgb01, out01, "Original (RGB)", f"{args.filter}-{args.type} (per-channel)")
        # For spectra, show luminance channel for readability
        gray01 = normalize01(cv2.cvtColor((rgb01*255).astype(np.uint8), cv2.COLOR_RGB2GRAY).astype(np.float64))
        _, Hmask_g, Fg, Ffg = process_gray(gray01, filt=args.filter, ftype=args.type,
                                           cutoff=args.cutoff, cutoff2=args.cutoff2,
                                           order=args.order, do_pad=args.pad)
        fig2 = panel_spectrum_and_mask(Fg, Hmask_g, Ffg, f"{args.filter}-{args.type} (luma view)")
        if args.save:
            save_rgb(outdir/(path.stem+"_orig_rgb.png"), rgb01)
            save_rgb(outdir/(path.stem+f"_{args.filter}_{args.type}_rgb.png"), out01)
            fig1.savefig(outdir/(path.stem+f"_{args.filter}_{args.type}_rgb_pair.png"), dpi=180, bbox_inches="tight")
            fig2.savefig(outdir/(path.stem+f"_{args.filter}_{args.type}_rgb_spectra.png"), dpi=180, bbox_inches="tight")
        plt.show()
