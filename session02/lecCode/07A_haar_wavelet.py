...
def normalize01(x: np.ndarray):
    ...
    mn, mx = x.min(), x.max()
    if mx <= mn: return np.zeros_like(x, dtype=np.float64)
    return (x - mn) / (mx - mn)
...
def synthetic_canvas(h=256, w=320):
    ...
    for _ in range(100):
        y = int(rng.uniform(0,h)); x = int(rng.uniform(0,w))
        img[y, x] = 50
    return img
...
def response_map(S: np.ndarray, w: int, h: int, ftype: str, stride: int = 1) -> np.ndarray:
...
def draw_topk_boxes(gray: np.ndarray, R: np.ndarray, w: int, h: int, stride: int, k: int, pos=True):
    """
    Draw k top responses (pos=True for maxima, False for minima).
    """
    Hmap, Wmap = R.shape
    flat_idx = np.argpartition((-R if pos else R).ravel(), kth=min(k-1, R.size-1))[:k]
...
def parse_args():
    ap = argparse.ArgumentParser(description="Haar-like features (Viola–Jones) + integral image")
    src = ap.add_mutually_exclusive_group(required=True)
    src.add_argument("--image", type=str, help="Path to grayscale/RGB image")
    src.add_argument("--synthetic", action="store_true", help="Use synthetic test canvas")
    ap.add_argument("--win", type=int, nargs=2, default=[24,24], metavar=("W","H"), help="Haar window size (w h)")
    ap.add_argument("--stride", type=int, default=1, help="Sliding step (>=1)")
    ap.add_argument("--topk", type=int, default=5, help="Draw top-k detections (max responses)")
    ap.add_argument("--save", action="store_true", help="Save outputs to ./haar_outputs")
    ap.add_argument("--outdir", type=str, default=None)
    ap.add_argument("--show-dwt", action="store_true", help="Also render 1-level 2D Haar (LL/LH/HL/HH)")
    return ap.parse_args()


def main():
    args = parse_args()
    outdir = Path(args.outdir or "haar_outputs"); outdir.mkdir(parents=True, exist_ok=True)
    ...
    for f in features:
        ...
        overlay = cv2.addWeighted(cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR), 0.6, Hres, 0.8, 0)
    ...
    fig.tight_layout()
    if args.save:
        fig.savefig(outdir / f"{name}_haar_heatmaps_grid.png", dpi=180, bbox_inches="tight")
