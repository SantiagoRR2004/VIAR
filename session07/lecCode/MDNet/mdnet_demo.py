...
class MDNet(nn.Module):
    ...
    def __init__(self, num_domains=1, init_weights=True):
        ...
        self.fc4 = nn.ModuleList(
            [nn.Linear(512 * 3 * 3, 512) for _ in range(num_domains)]
        )
...
class MDNetTracker:
    ...
    def init(self, image, bbox):
        ...
        pos_samples = self._sample_patches(
            image, bbox, n_samples=50, trans_f=0.1, scale_f=1.0
        )
        neg_samples = self._sample_patches(
            image, bbox, n_samples=200, trans_f=2.0, scale_f=1.0
        )
    ...
    def update(self, image):
        ...
        candidates = self._sample_patches(
            image,
            self.target_bbox,
    ...
    def _train_initial(self, pos_samples, neg_samples):
        ...
        y = torch.cat(
            [torch.ones(len(pos_samples)), torch.zeros(len(neg_samples))]
        ).long()
    ...
    def _update_online(self, image, bbox):
        ...
        pos_samples = self._sample_patches(
            image, bbox, n_samples=20, trans_f=0.1, scale_f=1.0
        )
        ...
        self.model.train()
        optimizer = torch.optim.SGD(
            self.model.fc6.parameters(), lr=self.lr * 0.1, momentum=0.9
        )
        criterion = nn.CrossEntropyLoss()
