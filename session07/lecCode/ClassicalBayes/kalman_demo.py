...
def main():
    ...
    parser.add_argument(
        "--dt", type=float, default=1.0, help="Time step (inverse of fps)"
    )
    parser.add_argument(
        "--process_noise", type=float, default=1e-2, help="Process noise covariance"
    )
    parser.add_argument(
        "--measurement_noise",
        type=float,
    ...
    parser.add_argument(
        "--output", type=str, default=None, help="Output video file (optional)"
    )
    parser.add_argument(
        "--manual",
        action="store_true",
    ...
    print(
        f"Process noise: {args.process_noise}, Measurement noise: {args.measurement_noise}"
    )
    ...
    while True:
        ...
        if args.manual:
            ...
            measurement = detect_object_simple(
                frame, tracker.corrected_state if tracker.initialized else None
            )
        ...
        if tracker.initialized:
            ...
            if predicted_pos is not None:
                ...
                draw_uncertainty_ellipse(
                    vis_frame, predicted_pos, angle, axis_length, (255, 255, 0)
                )
        ...
        if measurement is not None:
            ...
            if velocity is not None:
                ...
                cv2.arrowedLine(
                    vis_frame, corr_pt, tuple(map(int, vel_end)), (255, 0, 255), 2
                )
        else:
