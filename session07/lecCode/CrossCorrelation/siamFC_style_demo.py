...
class TinyBackbone(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=32):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(in_ch, 32, 11, stride=2, padding=0),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(3, stride=2),  # big early downsample
            nn.Conv2d(32, 32, 5, stride=1, padding=0),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, out_ch, 3, stride=1, padding=0),
...
if __name__ == "__main__":
    ...
    print(
        f"[OpenCV] GT (r,c)=({gt_r},{gt_c}) | Pred=({cv_pred_r},{cv_pred_c}) | Peak={max_val:.3f}"
    )
    ...
    print(
        f"[SiamFC-PyTorch] Response shape: {tuple(response.shape)} | Peak index in response map: (r={r_peak}, c={c_peak})"
    )
