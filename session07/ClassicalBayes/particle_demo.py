...
class ParticleFilterTracker:
    ...
    def init_tracking(self, frame, bbox):
        ...
        self.particles[:, 0] = np.random.normal(
            center_x, w / 4, self.num_particles
        )  # x
        self.particles[:, 1] = np.random.normal(
            center_y, h / 4, self.num_particles
        )  # y
    ...
    def predict(self):
        ...
        self.particles[:, 0] += self.particles[:, 2] * self.dt + np.random.normal(
            0, self.process_std[0], self.num_particles
        )
        self.particles[:, 1] += self.particles[:, 3] * self.dt + np.random.normal(
            0, self.process_std[1], self.num_particles
        )
        self.particles[:, 2] += np.random.normal(
            0, self.process_std[2], self.num_particles
        )
        self.particles[:, 3] += np.random.normal(
            0, self.process_std[3], self.num_particles
        )
    ...
    def update(self, frame):
        ...
        for i in range(self.num_particles):
            self.weights[i] = self.compute_observation_likelihood(
