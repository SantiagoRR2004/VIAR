...
def make_clips(
    frames: list[np.ndarray], clip_len=16, stride=16, max_clips=None
) -> list[np.ndarray]:
    T = len(frames)
    clips = []
...
def load_ucf101_labels(classInd: str | None, n=101) -> list[str]:
    if not classInd or not os.path.isfile(classInd):
...
def main():
    ...
    ap.add_argument(
        ...
        required=False,
        default=None,
        help="Path to C3D checkpoint (.pickle/.pth)",
    )
    ap.add_argument(
        "--labels",
        required=False,
        default=None,
        help="Sports-1M labels.txt (487) OR UCF101 classInd.txt (101)",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=101,
        help="487 for Sports-1M prediction, 101 for UCF101/fine-tune",
    )
