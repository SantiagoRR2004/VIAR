...
class C3D(nn.Module):
    def __init__(self, num_classes: int = 101, dropout: float = 0.5):
        ...
        self.conv1 = nn.Conv3d(3,   64, 3, padding=1); self.pool1 = nn.MaxPool3d((1,2,2), (1,2,2))
        self.conv2 = nn.Conv3d(64, 128, 3, padding=1); self.pool2 = nn.MaxPool3d((2,2,2), (2,2,2))
        self.conv3a= nn.Conv3d(128,256, 3, padding=1)
        self.conv3b= nn.Conv3d(256,256, 3, padding=1); self.pool3 = nn.MaxPool3d((2,2,2), (2,2,2))
        self.conv4a= nn.Conv3d(256,512, 3, padding=1)
        self.conv4b= nn.Conv3d(512,512, 3, padding=1); self.pool4 = nn.MaxPool3d((2,2,2), (2,2,2))
        self.conv5a= nn.Conv3d(512,512, 3, padding=1)
        self.conv5b= nn.Conv3d(512,512, 3, padding=1); self.pool5 = nn.MaxPool3d((2,2,2), (2,2,2))
...
def make_clips(frames: List[np.ndarray], clip_len: int = 16, stride: int = 16, max_clips: int | None = None) -> List[np.ndarray]:
    clips = []
    T = len(frames)
...
def main():
    ...
    ap.add_argument("--weights", default=None, type=str, help="Optional C3D checkpoint (.pth)")
    ap.add_argument("--labels", default=None, type=str, help="Optional classInd.txt (UCF101)")
    ...
    if args.weights:
        ...
        print("No weights provided: using ImageNet-like init (random) — predictions will be untrained.")
    ...
    for r, (p, idx) in enumerate(zip(topk.values, topk.indices), 1):
        name = labels[idx.item()] if 0 <= idx.item() < len(labels) else f"class_{idx.item()}"
        print(f"  {r}. {name:<30} {float(p)*100:5.2f}%")

    print("="*80)
    print("Tip: supply --weights (fine-tuned on UCF101) and --labels (classInd.txt) for meaningful actions.")
    print("="*80)
