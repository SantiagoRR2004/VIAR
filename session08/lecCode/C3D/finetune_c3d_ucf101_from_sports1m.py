...
class C3D_Sports1M_8192(nn.Module):
    ...
    def forward(self, x):
        # x: [B,3,16,112,112]
        h = self.relu(self.conv1(x)); h = self.pool1(h)
        h = self.relu(self.conv2(h)); h = self.pool2(h)
        h = self.relu(self.conv3a(h)); h = self.relu(self.conv3b(h)); h = self.pool3(h)
        h = self.relu(self.conv4a(h)); h = self.relu(self.conv4b(h)); h = self.pool4(h)
        h = self.relu(self.conv5a(h)); h = self.relu(self.conv5b(h)); h = self.pool5(h)
...
def load_c3d_pickle_into_model(model, ckpt_path, drop_fc8_if_mismatch: bool = True):
    ...
    ckpt = None
    try:
        ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=True)  # torch>=2.4
    except TypeError:
        try:
...
# UCF101 list parsing
# ----------------------------
def parse_lists(root: Path, split: int) -> Tuple[List[Tuple[Path,int]], List[Tuple[Path,int]]]:
    listdir = root / "ucfTrainTestlist"
    cls_map = {ln.split()[1]: int(ln.split()[0])-1
               for ln in (listdir / "classInd.txt").read_text().splitlines() if ln.strip()}
    train, test = [], []
...
def train_one_epoch(model, loader, opt, dev, log_interval=25, amp=False):
    ...
    for b, (xK, y) in enumerate(loader, start=1):
        ...
        with torch.no_grad(): acc = accuracy_top1(logits_mean, y)
        loss_sum += loss.item() * B; acc_sum += acc * B; total += B

        if b == 1 or b % log_interval == 0:
            elapsed = time.time() - t0
            eta = elapsed / b * (n_batches - b)
            print(f"[Train] {b:4d}/{n_batches:4d} | loss {loss.item():.4f} | "
...
def evaluate(model, loader, dev, log_interval=100, amp=False):
    ...
    for b, (xK, y) in enumerate(loader, start=1):
        ...
        acc_sum += acc * B; total += B

        if b == 1 or b % log_interval == 0:
            elapsed = time.time() - t0
            print(f"[Val]   {b:4d}/{n_batches:4d} | avg_acc_so_far {(acc_sum/total):5.2f}% | elapsed {elapsed:.1f}s")
...
def set_freeze_policy(model: nn.Module, policy: str):
    ...
    if policy == "fc":
        for p in list(model.fc6.parameters()) + list(model.fc7.parameters()) + list(model.fc8.parameters()):
...
def main():
    ...
    ap.add_argument("--weights", type=str, required=True, help="Sports-1M C3D checkpoint (.pickle/.pth)")
    ...
    ap.add_argument("--freeze", type=str, default="fc", choices=["none","fc","conv5","all"])
    ...
    print(f"[Setup] Train {len(train_ds)} | Val {len(val_ds)} | seg={args.segments} | bs={args.batch_size} "
          f"| freeze={args.freeze} | amp={args.amp}")

    best = 0.0
    for ep in range(1, args.epochs+1):
        tr_loss, tr_acc = train_one_epoch(model, train_ld, opt, dev, log_interval=args.log_interval, amp=args.amp)
        va_acc = evaluate(model, val_ld, dev, log_interval=args.log_interval*2, amp=args.amp)
        print(f"[Epoch] {ep:02d}/{args.epochs} | Train loss {tr_loss:.4f} | Train acc {tr_acc:.2f}% | Val acc {va_acc:.2f}%")

        ckpt = {"epoch": ep, "state_dict": model.state_dict(), "optimizer": opt.state_dict()}
