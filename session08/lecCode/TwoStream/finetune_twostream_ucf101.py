...
def parse_train_test_lists(root: Path, split: int) -> Tuple[List[Tuple[Path,int]], List[Tuple[Path,int]], List[str]]:
...
# -----------------------
def tvl1_available():
    return hasattr(cv2, "optflow") and hasattr(cv2.optflow, "DualTVL1OpticalFlow_create")

def compute_flow_pair(prev_gray: np.ndarray, next_gray: np.ndarray, method: str = "auto") -> np.ndarray:
    if method == "tvl1" or (method == "auto" and tvl1_available()):
        ...
        flow = cv2.calcOpticalFlowFarneback(prev_blr, next_blr, None,
                                            pyr_scale=0.5, levels=3, winsize=25,
                                            iterations=5, poly_n=7, poly_sigma=1.5, flags=0)
    return flow.astype(np.float32)  # HxWx2

def flow_stack_from_frames(frames_gray: List[np.ndarray], center: int, num_pairs: int,
                           out_size: int = 224, method: str = "auto") -> np.ndarray:
...
def flow_cache_key(video_path: Path, center: int, num_pairs: int, size: int, method: str) -> str:
...
class VGGStream(nn.Module):
    def __init__(self, in_channels: int = 3, num_classes: int = 101, inflate_from_rgb: bool = False):
        super().__init__()
...
class TwoStreamNet(nn.Module):
    def __init__(self, num_classes: int = 101):
        ...
        self.temporal = VGGStream(in_channels=20, num_classes=num_classes, inflate_from_rgb=True)
...
class UCFTwoStreamDataset(Dataset):
    def __init__(self, items: List[Tuple[Path,int]], class_names: List[str],
                 segments: int = 3, num_pairs: int = 10, size: int = 224,
                 split: str = "train", flow_method: str = "auto",
                 cache_dir: Path | None = None, spatial_on: bool = False):
        """
    ...
    def _sample_centers(self, T: int) -> np.ndarray:
        ...
        idxs = np.linspace(self.num_pairs//2, T - self.num_pairs//2 - 1, num=self.segments)
    ...
    def __getitem__(self, i):
        ...
        for c in centers:
            ...
            if self.cache_dir is not None:
                ...
                key = flow_cache_key(vpath, int(c), self.num_pairs, self.size, self.flow_method)
                cache_file = self.cache_dir / key
                if cache_file.exists():
                    flow_arr = np.load(cache_file)
            if flow_arr is None:
                flow_arr = flow_stack_from_frames(frames_gray, int(c), self.num_pairs, self.size, self.flow_method)
                if self.cache_dir is not None:
        ...
        rgb_batch = torch.stack(rgb_tensors, dim=0) if self.spatial_on else torch.empty(0)
...
def train_one_epoch(model: TwoStreamNet, loader: DataLoader, opt, dev, args):
    ...
    for bidx, (rgbK, flowK, y) in enumerate(loader, start=1):
        ...
        if bidx % args.log_interval == 0 or bidx == 1:
            ...
            print(f"[Train] batch {bidx:4d}/{len(loader):4d} | "
...
def evaluate(model: TwoStreamNet, loader: DataLoader, dev, args):
    ...
    for bidx, (rgbK, flowK, y) in enumerate(loader, start=1):
        ...
        if bidx % max(1, (args.log_interval*2)) == 0 or bidx == 1:
            ...
            print(f"[Val]   batch {bidx:4d}/{len(loader):4d} | "
...
def main():
    ...
    parser.add_argument("--num_pairs", type=int, default=10, help="(u,v) pairs per flow stack")
    parser.add_argument("--size", type=int, default=224)
    parser.add_argument("--flow_method", type=str, default="auto", choices=["auto","tvl1","farneback"])
    ...
    parser.add_argument("--temporal_only", action="store_true", help="train only temporal stream (default if set)")
    parser.add_argument("--spatial", action="store_true", help="also include spatial stream data")
    parser.add_argument("--fuse", action="store_true", help="optimize fused logits (requires spatial+temporal)")
    parser.add_argument("--unfreeze_spatial", action="store_true", help="train spatial stream too")
    parser.add_argument("--unfreeze_temporal_backbone", action="store_true", help="unfreeze more of temporal backbone")
    ...
    train_ds = UCFTwoStreamDataset(train_items, class_names, segments=args.segments,
                                   num_pairs=args.num_pairs, size=args.size,
                                   split="train", flow_method=args.flow_method,
                                   cache_dir=cache_dir, spatial_on=args.spatial)
    val_ds   = UCFTwoStreamDataset(test_items, class_names, segments=args.segments,
                                   num_pairs=args.num_pairs, size=args.size,
                                   split="val", flow_method=args.flow_method,
                                   cache_dir=cache_dir, spatial_on=args.spatial)

    train_ld = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
    val_ld   = DataLoader(val_ds,   batch_size=max(1,args.batch_size//2), shuffle=False, num_workers=4, pin_memory=True)
    
    print(f"[Setup] Train videos: {len(train_ds)} | Val videos: {len(val_ds)} "
    ...
    for p in model.temporal.backbone.classifier.parameters(): p.requires_grad = True
    ...
    if args.unfreeze_spatial:
        for p in model.spatial.backbone.classifier.parameters(): p.requires_grad = True
    ...
    opt = torch.optim.SGD(params, lr=args.lr, momentum=args.momentum, weight_decay=args.wd)

    ensure_dir(args.checkpoint_dir)
    best_acc = 0.0

    print(f"Train videos: {len(train_ds)} | Val videos: {len(val_ds)} | Temporal-only: {args.temporal_only} | Spatial used: {args.spatial} | Fuse: {args.fuse}")
    print(f"Flow: {args.flow_method} | segments: {args.segments} | pairs: {args.num_pairs}")


    print("[Info] Starting training … computing optical flow & building batches (first epoch may be slow).")
    ...
    for epoch in range(1, args.epochs+1):
        ...
        print(f"[Epoch] {epoch:02d}/{args.epochs} | {ep_time:.1f}s | "
            f"Train loss {tr_loss:.4f} | Train acc {tr_acc:.2f}% | Val acc {va_acc:.2f}%")
        dt = time.time() - t0
        print(f"Epoch {epoch:02d}/{args.epochs} | {dt:.1f}s | Train loss {tr_loss:.4f} | Train acc {tr_acc:.2f}% | Val acc {va_acc:.2f}%")
        ...
        torch.save(ckpt, os.path.join(args.checkpoint_dir, f"ckpt_epoch{epoch:02d}.pth"))
        if is_best:
