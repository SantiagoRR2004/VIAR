...
def compute_flow_stack(
    ...
    flow_cropped = flow_resized[ch:ch+out_size, cw:cw+out_size, :].astype(np.float32)
...
class VGGStream(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, num_classes: int = 1000, inflate_from_rgb: bool = False):
        super().__init__()
        ...
        if in_channels != 3:
            ...
            if inflate_from_rgb:
                ...
                with torch.no_grad():
                    ...
                    w_rep = w_avg.repeat(1, in_channels, 1, 1) * (3.0 / float(in_channels))
...
class TwoStreamNet(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, rgb: torch.Tensor, flow20: torch.Tensor, fusion: str = "late") -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        rgb:  [B,3,224,224]
...
def read_video_frames(path: str, max_frames: int = 64) -> List[np.ndarray]:
    ...
    try:
        ...
        cap.release()
    if len(frames) < 12:
        raise RuntimeError("Video too short for a clean demo (need at least ~12 frames).")
    return frames
...
# Inference helpers
# -----------------------------
def topk_readable(logits: torch.Tensor, labels: List[str], k: int = 5) -> List[Tuple[str, float]]:
...
def main():
    ...
    parser.add_argument("--video", type=str, required=True, help="Path to an input video file")
    parser.add_argument("--device", type=str, default="cuda", choices=["cuda", "cpu"])
    parser.add_argument("--max_frames", type=int, default=96, help="Max frames to read")
    parser.add_argument("--num_flow_pairs", type=int, default=10, help="TV-L1 flow pairs (u,v) -> 20 channels")
    parser.add_argument("--size", type=int, default=224, help="Input crop size")
    # in argparse section
    parser.add_argument("--flow", type=str, default="auto", choices=["auto", "tvl1", "farneback"],
    ...
    rgb_tensor = prepare_rgb_tensor(frames_bgr[mid], rgb_tfm).unsqueeze(0)  # [1,3,224,224]
    ...
    print(f"Flow window starts at frame {stats['window_start']} "
        f"| mean mag per-pair: {[f'{m:.3f}' for m in stats['pair_mean_mag']]}")
    print(f"Global mean mag: {stats['global_mean']:.3f} | global max: {stats['global_max']:.3f}")
    ...
    print("\nNotes:")
    print("- This demo shows the mechanics of Two-Stream inference with real optical flow.")
    print("- For *action* labels, fine-tune on UCF101/Kinetics and set num_classes accordingly.")
    print("- Temporal first conv is inflated from RGB conv weights to accept 20 channels (flow stack).")
    print("="*80)
